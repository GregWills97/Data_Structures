...
List::List(const List& rhs) // Copy constructor - homework 
{
   if(rhs.first == nullptr){
      first = nullptr;
      last = nullptr;		//empty list
   }
   else{
      Iterator i;

      i.position = rhs.first;
      Node* head_node = new Node(i.get()); //establish first node
      first = head_node;
      last = head_node;			   //(maybe the only)

      i.next();
      
      while(i.position != nullptr) {
         Node* new_node = new Node(i.get());
         new_node->previous = last;
	 last->next = new_node;		   //pushing back new nodes into the list
	 last = new_node;
	 i.next();	 
      }
   }
